...
<template>
  <div class="bg-white rounded-[10px] border border-border shadow-sm overflow-hidden">
    <div class="px-5 py-4 border-b border-border flex items-center gap-2">
      ...
      <h3 class="text-sm font-display font-medium text-text-primary">
    ...
    <div class="overflow-x-auto">
      <table class="w-full">
        <thead class="bg-surface-table">
          <tr>
            <th class="text-left px-5 py-3 text-xs font-display font-medium text-text-muted">
              Timestamp
            </th>
            <th class="text-left px-5 py-3 text-xs font-display font-medium text-text-muted">
              Action
        ...
        <tbody>
          ...
          >
            <td class="px-5 py-3 text-sm text-text-secondary font-sans whitespace-nowrap">
              {{ activity.timestamp }}
            </td>
            <td
              class="px-5 py-3 text-sm font-sans font-medium"
              :class="actionColor[activity.action] ?? 'text-text-primary'"
            >
              {{ activity.action }}
            </td>
          ...
          <tr v-if="!activities.length">
            <td colspan="3" class="px-5 py-8 text-center text-sm text-text-muted font-sans">
              No recent activity.
            </td>
          </tr>
        </tbody>
