...
- **Counted messages: `ar` runs a CLDR six-category plural rule** (`arabicPluralRule`, `src/app/plugins/i18n.ts`). Any pluralized Arabic message must carry **exactly six** forms, pipe-separated, in `zero | one | two | few | many | other` order — `few` is `n % 100` 3–10, `many` is `n % 100` 11–99, `other` is everything else (100, 101, 200…). Fewer than six forms falls back to vue-i18n's English-shaped default rule **silently**, with no lint error and no test failure, which is the bug this convention exists to prevent. The matching English message carries 1–3 forms (`singular | plural`); an `en` message must never have more than three, because the default rule pins every count ≥ 2 to index 2.
- **A plural call site must hand `t()` a real `number`.** vue-i18n only auto-detects the plural index from a named `count`/`n` that is a JavaScript number — a pre-formatted, thousands-separated string silently selects the _one_ form. Pass the display value and the selection value separately: `t(key, { count: formatCount(n, locale) }, n)`. The same applies when the placeholder is not called `count` at all (`{total}`, `{max}`, `{submitted}`): auto-detection cannot fire, so the third argument is mandatory. See `BulkImportPage`, `BulkImportResultPanel`, `PipelineMonitorPage`, `AppFileUpload`.
- **A message carrying two independent counts cannot be pluralized** — one `t()` call selects one form. Word the Arabic so no numeral governs a noun (`قُبل {queued} من {submitted} من الملفات المرسلة`, `الصفوف: {processed} · ناجح: {success}`) rather than hard-coding whichever form happens to suit today's numbers. Same for a numeral that is a fixed constant: write the Arabic for that one CLDR bucket and leave a comment at the constant (see `POLL_INTERVAL_MS` in `PipelineMonitorPage`).
...
- **Narrow, explicit exception**: a `shared/` component _may_ call `useI18n()` to translate its own default labels/messages (`AppFileUpload`, `AppConfirmDialog`, `AppAsyncSelect`, `AppToastHost`, `DataTable`). This does not violate the boundary rule — `useI18n()` is imported straight from the `vue-i18n` package, never from `src/app/plugins/i18n.ts`, and it resolves against the single global instance `main.ts` installs with `app.use(i18n)` via Vue's own provide/inject, the same way any module component's `t()` does. What stays off-limits is importing the `i18n`/`setLocale` exports themselves and anything that requires `src/app/` (a store's fallback text, the locale switch itself) — those keep using the existing patterns (`i18n.global.t()` from a store; an emitted event for a locale change). Prefer this over threading every label through as a prop: a `shared/` component with its own copy (rejection reasons, "Loading…", a confirm button's default) should default to a translated string the same way a module component would, and a caller that truly needs different wording still overrides it via a prop.
- `getApiErrorMessage(err, fallback)` (`src/shared/utils/apiError.ts`) is a plain function, not a component, so it cannot call `useI18n()` either — its bare default stays the English literal `'Something went wrong'`. Every production call site passes a `t(...)`/`i18n.global.t(...)` fallback, so the literal is unreachable in practice; keep it that way rather than threading a translator through every call site for a string nobody sees.
